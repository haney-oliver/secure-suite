...
def fetch_url_data():
    ...
    struct_data = pd.DataFrame(filter(None,[get_lookup_data(url[0],url[1]) for url in progressbar.progressbar(urls)]), columns=['url', 'url_domain', 'url_suffix', 'whois_registrar', 'whois_domain', 'whois_suffix', 'whois_org', 'url_domain_ip', 'label'])
    struct_data.to_csv(Path("data/") / "struct_data.csv", encoding='utf-8', index=False)
