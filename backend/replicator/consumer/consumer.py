...
try:
  for msg in consumer:
    ...
    dbp.execute("CREATE TABLE IF NOT EXISTS {}_events (`event_key` VARCHAR(36) NOT NULL, `table` VARCHAR(255) NOT NULL, `column` VARCHAR(255), `data` TEXT, `event_type` VARCHAR(255) NOT NULL, `commited` BOOLEAN NOT NULL, `xid` VARCHAR(255) NOT NULL, `timestamp` VARCHAR(255), PRIMARY KEY (`event_key`))".format(database_name))
