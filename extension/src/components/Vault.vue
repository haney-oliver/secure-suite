<template>
  <span class="vault">
    <navbar />
    <h2 class="user-name">{{ user.user_name }}'s VAULT</h2>
    <div class="message">
      <h4 v-if="zeroCategories">
        You have no passwords. Click "Add New Password" above to get started.
      </h4>
    </div>
    <category
...
export default {
  ...
  methods: {
    fetchCategories() {
      console.log(this.session_key, this.user.user_key);
      axios
  ...
  mounted() {
    ...
    console.log(this.user, this.session_key);
    EventBus.$on("password-clicked", (password) => {
